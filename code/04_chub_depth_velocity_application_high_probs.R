...
for(n in 1: length(h)) {
  ...
  write.csv(limits, paste("output_data/04_",NodeName,"_Chub_Adult_depth_Q_limits_High_Probs.csv", sep=""))
  ## percentage time
  ...
  write.csv(melt_time, paste("output_data/04_", NodeName, "_Chub_Adult_depth_time_stats_High_Probs.csv", sep=""))
  ...
  ## save df
  write.csv(melt_days, paste("output_data/04_", NodeName, "_Chub_Adult_depth_total_days_long_High_Probs.csv", sep="") )
...
for(n in 1: length(h)) {
  ...
  for(p in 1:length(positions)) {
    ...
    ## find roots for each probability
    newx1 <- RootLinearInterpolant(new_data$Q, new_data$prob_fit, 0.3)
    hy_lim <- RootLinearInterpolant(new_data$vel_m, new_data$prob_fit, 0.3)
  ...
  write.csv(limits, paste("output_data/04_",NodeName,"_Chub_Adult_Velocity_Q_limits_High_Probs.csv", sep=""))
  ## percentage time
  melt_time<-reshape2::melt(time_statsx, id=c("season", "position", "water_year", "Node"))
  melt_time <- melt_time %>% 
    rename( Probability_Threshold = variable) %>%
  ...
  write.csv(melt_time, paste("output_data/04_", NodeName, "_Chub_Adult_Velocity_time_stats_High_Probs.csv", sep=""))
  rm(melt_time)
  rm(time_statsx)
  ### days per month
  days_data <- select(days_data, c(Q, month, water_year, day, ID01, Mid, position, DateTime, Node) )# all probs
  ...
  ## save df
  write.csv(melt_days, paste("output_data/04_", NodeName, "_Chub_Adult_Velocity_total_days_long_High_Probs.csv", sep="") )

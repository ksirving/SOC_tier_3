...
for(n in 1: length(h)) {
  ...
  write.csv(limits, paste("output_data/02_",NodeName,"_Willow_Seedling_depth_Q_limits_High_Probs.csv", sep=""))
  ## percentage time
  ...
  write.csv(melt_time, paste("output_data/02_", NodeName, "_Willow_Seedling_depth_time_stats_High_Probs.csv", sep=""))
  ...
  ## save df
  write.csv(melt_days, paste("output_data/02_", NodeName, "_Willow_Seedling_depth_total_days_long_High_Probs.csv", sep="") )
...
for(n in 1: length(h)) {
  ...
  write.csv(limits, paste("output_data/02_",NodeName,"_Willow_Seedling_shear_Q_limits_High_Probs.csv", sep=""))
  ## percentage time
  melt_time<-reshape2::melt(time_statsx, id=c("season", "position", "water_year", "Node"))
  melt_time <- melt_time %>% 
    rename( Probability_Threshold = variable) %>%
    mutate(Species ="Willow", Life_Stage = "Seedling", Hydraulic = "Shear Stress", Node = NodeName)
  head(melt_time)
  write.csv(melt_time, paste("output_data/02_", NodeName, "_Willow_Seedling_shear_time_stats_High_Probs.csv", sep=""))
  ...
  ## save df
  write.csv(melt_days, paste("output_data/02_", NodeName, "_Willow_Seedling_shear_total_days_long_High_Probs.csv", sep="") )
